...
public class UI_Result02 : UI_Popup
{
    enum GameObjects
    {
        Content
    ...
    public override void Init()
    {
        ...
        GetText((int)Texts.TxtTitle).text = $"{Managers.Data.CurUser} - 실험 2 결과";
    ...
    private void CreateExperimentDetailViews(GameObject parent)
    {
        ...
        Managers.Data.ColorDict.TryGetValue(Managers.Data.CurUser, out colorList);
        for (int i = 0; i < 8; i++)
        {
            GameObject detail = Managers.Resource.Instantiate("UI/SubItem/UI_ResultDetailView");
            detail.transform.SetParent(parent.transform);
            ...
            int color = colorList[i + (8 * (page - 5))];
            if (color == 0 || color == 1)
                detailSlot.Setup("연보라");
            else if (color == 2 || color == 3)
                detailSlot.Setup("진보라");
            else if (color == 4 || color == 5)
                detailSlot.Setup("연노랑");
            else if (color == 6 || color == 7)
                detailSlot.Setup("진노랑");
        }
